...
class HS_G(nn.Module):
    def __init__(self,input_ref, input_enc, RGB):
        super().__init__()
        ...
        in_color_ch = 3 if RGB else 1

        FeatureBlock_enc = [Conv(in_color_ch, self.n_colors, kernel_size, padding=2, act=True),
                        ResBlock(Conv, self.n_colors, kernel_size, padding=2),
                        ResBlock(Conv, self.n_colors, kernel_size, padding=2),
                        ResBlock(Conv, self.n_colors, kernel_size, padding=2)]

        FeatureBlock_ref = [Conv(in_color_ch, n_feats1, kernel_size, padding=2, act=True),
                        ResBlock(Conv, n_feats1, kernel_size, padding=2),
                        ResBlock(Conv, n_feats1, kernel_size, padding=2),
                        ResBlock(Conv, n_feats1, kernel_size, padding=2)]

        InBlock1 = [Conv(self.n_colors * self.input_enc + self.input_ref * n_feats1 + self.input_enc * n_feats1, n_feats, kernel_size, padding=2, act=True),
                   ResBlock(Conv, n_feats, kernel_size, padding=2),
                   ResBlock(Conv, n_feats, kernel_size, padding=2),
                   ResBlock(Conv, n_feats, kernel_size, padding=2)]
        InBlock2 = [Conv(self.n_colors * self.input_enc + self.input_ref * n_feats1 + self.input_enc * n_feats1 + n_feats, n_feats, kernel_size, padding=2, act=True),
                   ResBlock(Conv, n_feats, kernel_size, padding=2),
                   ResBlock(Conv, n_feats, kernel_size, padding=2),
                   ResBlock(Conv, n_feats, kernel_size, padding=2)]
    ...
    def forward(self, capture, kernel):
        ...
        for i in range(capture.shape[0]):
            if i<self.input_ref:
                ref_feature_out_i = self.FeatureBlock_ref(capture[i])
                ref_feature_out.append(ref_feature_out_i)
            else:
                enc_feature_out_hs_i = self.FeatureBlock_enc(capture[i])
                enc_feature_out_i = self.FeatureBlock_ref(capture[i])
                enc_clear_features_i = Wiener_deconv(enc_feature_out_hs_i, kernel[i].unsqueeze(0))

                enc_feature_out.append(enc_feature_out_i)
                enc_clear_features.append(enc_clear_features_i)

        enc_feature_out = torch.cat(enc_feature_out,dim=1)
        ref_feature_out = torch.cat(ref_feature_out,dim=1)
        enc_clear_features = torch.cat(enc_clear_features,dim=1)
